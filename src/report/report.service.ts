...
export class ReportService {
  ...
  async generate(generateReportDto: GenerateReportDto) {
    ...
    const totalDuration = calls.reduce((sum: number, call: { durationSec: number | null }) => sum + (call.durationSec || 0), 0);
    
    const allScores = calls.flatMap((call: { scores: { score: number }[] }) => call.scores.map((score: { score: number }) => score.score));
